...
function Start({ provider, account }: { provider: WalletChoice['provider'] | null; account: `0x${string}` | null }) {
  ...
  const [proposalAt, setProposalAt] = useState(() => localDateTime(5 / 60));
  const [recoveryAt, setRecoveryAt] = useState(() => localDateTime(10 / 60));
